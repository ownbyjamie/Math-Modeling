...
void getForces()
{
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		for(int j = i + 1; j < NUMBER_OF_BALLS; j++)
		{
			...
			if (distance < minDistance) // this is when the balls are colliding
			{
				float overlap = minDistance - distance;
				float normalVectorX = dx / distance;
				float normalVectorY = dy / distance;
				float normalVectorZ = dz / distance;

				float relativeVelocityX = Velocity[i].x - Velocity[j].x;
				float relativeVelocityY = Velocity[i].y - Velocity[j].y;
				float relativeVelocityZ = Velocity[i].z - Velocity[j].z;
				float velocityAlongNormal = relativeVelocityX * normalVectorX + relativeVelocityY * normalVectorY + relativeVelocityZ * normalVectorZ;

				//if (velocityAlongNormal > 0)
				//	continue;

				//float restitution = 0.8; // elasticity
				float impulse = (2 * velocityAlongNormal)/(SphereMass + SphereMass);

				//Force[i].x = Force[i].x - (impulse * normalVectorX * SphereMass);
				//Force[i].y = Force[i].y - (impulse * normalVectorY * SphereMass);
				//Force[i].z = Force[i].z - (impulse * normalVectorZ * SphereMass);
				//Force[j].x = Force[j].x + (impulse * normalVectorX * SphereMass);
				//Force[j].y = Force[j].y + (impulse * normalVectorY * SphereMass);
				//Force[j].z = Force[j].z + (impulse * normalVectorZ * SphereMass);

				Velocity[i].x = Velocity[i].x - (impulse * normalVectorX * SphereMass);
				Velocity[i].y = Velocity[i].y - (impulse * normalVectorY * SphereMass);
				Velocity[i].z = Velocity[i].z - (impulse * normalVectorZ * SphereMass);
				Velocity[j].x = Velocity[j].x + (impulse * normalVectorX * SphereMass);
				Velocity[j].y = Velocity[j].y + (impulse * normalVectorY * SphereMass);
				Velocity[j].z = Velocity[j].z + (impulse * normalVectorZ * SphereMass);

				//Position[i].x = Position[i].x + (overlap*normalVectorX / 2);
				//Position[i].y = Position[i].y + (overlap*normalVectorY / 2);
				//Position[i].z = Position[i].z + (overlap*normalVectorZ / 2);
				//Position[j].x = Position[j].x - (overlap*normalVectorX / 2);
				//Position[j].y = Position[j].y - (overlap*normalVectorY / 2);
				//Position[i].z = Position[j].z - (overlap*normalVectorZ / 2);

				float correction = overlap / 2.0f;
				Position[i].x = Position[i].x + correction*normalVectorX;
				Position[i].y = Position[i].y + correction*normalVectorY;
				Position[i].z = Position[i].z + correction*normalVectorZ;
				Position[j].x = Position[j].x - correction*normalVectorX;
				Position[j].y = Position[j].y - correction*normalVectorY;
				Position[j].z = Position[j].z - correction*normalVectorZ;


				//if (velocityAlongNormal > 0) //updating velocities based on collision and ball movement
				//{
				//	Velocity[i].x = Velocity[i].x - (impulse*SphereMass*normalVectorX);
				//	Velocity[i].y = Velocity[i].y - (impulse*SphereMass*normalVectorY);
				//	Velocity[i].z = Velocity[i].z - (impulse*SphereMass*normalVectorZ);
				//	Velocity[j].x = Velocity[j].x + (impulse*SphereMass*normalVectorX);
				//	Velocity[i].y = Velocity[j].y + (impulse*SphereMass*normalVectorY);
				//	Velocity[i].z = Velocity[j].z + (impulse*SphereMass*normalVectorZ);
				//}
